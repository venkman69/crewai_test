...
def save_job_text(job_text: str, crew_result: CrewOutput, job_details: dict):
...
def job_vs_resume_analyzer_crew(
    job_analysis_crew_result: CrewOutput,
    resume_anaysis_crew_result: CrewOutput,
    job_details: dict,
    ...
    if (
        get_from_cache
        and (job_analysis_crew_result, resume_anaysis_crew_result) in utils.dc
    ):
        print("Using cached job vs resume result")
        cached_result = list(
            utils.dc[(job_analysis_crew_result, resume_anaysis_crew_result)]
        )
        cached_result.append(empty_crew_usage_metrics)
        return cached_result
    resume_raw_result = result_to_json(resume_anaysis_crew_result)
    job_raw_result = result_to_json(job_analysis_crew_result)
    input_data = {
        "resume_skills": resume_raw_result,
        "job_requirements": job_raw_result,
    }
    hr_crew = HRCrew().crew()
    start = utils.currenttimemillis()
    ...
    save_path = save_job_requirements_analysis(
        hr_crew, hr_result, job_details, "jobvsresume"
    )
    ...
    return hr_result, save_path, job_details, crew_usage_metrics
